...
{
    auto *new_node =
      reinterpret_cast<MallocBlock *>(node + size + sizeof(MallocBlock));
...
void init(const dts::u32 size)
{
    ...
    for (dts::u32 i = 0, virt = heap_start_virtual_address;
         i < malloc_page_count;
        ...
        Mem::set_flag(
          page, dts::to_underlying_type(Mem::PageFlags::READ_WRITE)
        );
    }

    if (list_head != nullptr) {
        list_head->block_size =
          (malloc_page_count * Mem::PAGE_SIZE) - sizeof(MallocBlock);
        list_head->free = true;
...
void *malloc(const dts::u32 size)
{
    ...
    if (current->block_size == size) {
        ...
        dts::u32 num_pages = 1;
        while (current->block_size + num_pages * Mem::PAGE_SIZE
               < size + sizeof(MallocBlock)) {
            ++num_pages;
        }

        dts::u32 virt =
          heap_start_virtual_address + malloc_page_count * Mem::PAGE_SIZE;
...
void merge_free_blocks()
{
    ...
    while (current != nullptr && current->next != nullptr) {
        if (current->free && current->next->free) {
            current->block_size +=
              (current->next->block_size + sizeof(MallocBlock));
...
void free(void *ptr)
{
    for (MallocBlock *current = list_head; current->next != nullptr;
         current              = current->next) {
